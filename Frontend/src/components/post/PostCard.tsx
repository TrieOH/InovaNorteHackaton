  ...
  return (
    <Link href={`/posts/${props.data.id}`} 
      className={cn(
        ...
        "w-full min-h-54 rounded-md px-5 py-4 cursor-pointer duration-500",
    ...
    >
      ...
      {/* Details */}
      <div className="flex flex-col w-0 h-full justify-between flex-1 gap-4"> 
        <div className="flex flex-col gap-3">
          <div>
            <h3 className="font-bold truncate">{props.data.title}</h3>
            <p className="max-w-[360px] text-xs flex items-center gap-1 overflow-hidden whitespace-nowrap truncate">
              ...
              <span>•</span>
              <span className="whitespace-nowrap">{timeAgo(props.data.created_at)}</span>
            </p>
          </div>
